...
def display_output(df):
    SalesID = df["Data"].iloc[0]  # Assuming the first column contains the product name
    api_url = "http://localhost:5000/api/scanned-data-receipt"  # Change if needed

    # Send scanned data to the API
    try:
        response = requests.post(api_url, json={"SalesID": SalesID})
        print(response.json())
